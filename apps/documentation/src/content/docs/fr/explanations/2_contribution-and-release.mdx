...
Cette page explique le raisonnement derrière notre système de contribution et de release. Les règles elles-mêmes vivent dans [`CONTRIBUTING.md`](https://github.com/lonestone/grocery/blob/main/CONTRIBUTING.md), et la mécanique (versions, releases, déploiements) est décrite sur [Release et versionnement](/grocery/fr/references/1_release_and_versionning).

:::note[Ce projet ajoute une branche d'intégration `staging`]
Le boilerplate standard merge chaque pull request directement sur `main`. Ce projet insère une étape : les pull requests de fonctionnalité visent **`staging`**, et `main` — le tronc de release — n'avance que par une **pull request de promotion** `staging` → `main` sans squash. L'environnement de déploiement `staging` builde depuis la branche `staging`. La cérémonie de curation (commits WIP → un commit de squash soigné par PR) est inchangée ; elle se joue simplement sur `staging` d'abord. Dans le récit ci-dessous, lisez « merge sur `main` » comme « merge sur `staging`, puis promotion vers `main` ». Le raisonnement derrière la branche en plus est dans la section « Pourquoi ce projet ajoute une branche d'intégration `staging` » plus bas.
:::
...
Pierrick développe la révocation de session sur une branche tirée de `staging`. Il committe au fil de l'eau, via son agent. Les commits sont des notes de travail honnêtes — l'un d'eux est littéralement une correction de faute de frappe. Chacun passe commitlint (lefthook vérifie le format en local), mais personne ne les peaufine. Ils n'atteindront jamais `staging` ni `main` en tant que commits.

```mermaid
gitGraph
  commit id: "chore(ci): 🔧 release 1.2.0" tag: "v1.2.0"
...
La Release PR ne bouge pas encore — release-please surveille `main`, et ce commit est encore sur `staging`. Il atteint `main` à la prochaine promotion (voir Vendredi), et c'est seulement là que la Release PR le prend en compte.
...
Son commit de squash atterrit sur `staging` lui aussi. Deux commits de fonctionnalité sont maintenant sur `staging`, en avance sur `main`.
...
Le client a validé les fonctionnalités sur staging ; Pierrick décide de livrer. Il promeut d'abord : il ouvre une pull request `staging` → `main` et la merge **sans squash**, pour que les deux commits de fonctionnalité atterrissent sur `main` individuellement (`main` avance en fast-forward jusqu'à `staging`). La Release PR se réveille alors — release-please lit les deux nouveaux commits, propose `1.3.0` (deux feats et un fix restent un mineur), et régénère `CHANGELOG.md` avec **trois** lignes : révocation, miniatures et le fix de pagination. Deux d'entre elles pointent vers le même commit `#147` ; c'est normal, le fix a été déclaré comme un changement à part entière.

Son travail restant est la partie humaine : il rédige la note de release (`releases/v1.3.0.mdx` dans l'app de documentation) — quelques phrases sur pourquoi cette release existe et ce qu'elle change pour les utilisateurs. Un check CI peut rendre cette note obligatoire ; le dépôt du boilerplate l'impose. Il merge la Release PR.
...
gitGraph
  ...
  commit id: "chore(ci): 🔧 release 1.3.0" tag: "v1.3.0"
```
...
### Pourquoi le squash merge

Un historique linéaire rend beaucoup plus facile de retrouver le commit qui a introduit un problème, et c'est ce que [release-please](https://github.com/googleapis/release-please) recommande pour analyser les changements de façon fiable. Il fait aussi de la pull request une unité de travail naturelle : aussi brouillons que soient les commits WIP, ce qui atterrit est un seul commit bien écrit — ce qui rend d'ailleurs le cherry-pick trivial quand il est un jour nécessaire.

Le boilerplate standard s'arrête là : un seul tronc, `main`, et chaque pull request y merge en squash. La « ligne stable » qu'une paire `develop`/`main` essaie de modéliser existe déjà comme la liste des tags, et release-please suppose un tronc unique.

### Pourquoi ce projet ajoute une branche d'intégration `staging`

Le modèle mono-tronc du boilerplate demande à chaque merge d'être prêt pour une release, parce que chaque merge *est* sur le tronc de release. Ce projet veut un endroit pour intégrer et valider plusieurs fonctionnalités ensemble — sur un vrai environnement déployé, devant la coopérative — avant qu'aucune ne compte pour une release. Donc `staging` est une vraie branche, pas juste un pointeur d'environnement : les pull requests de fonctionnalité y atterrissent (même cérémonie de squash, même commit soigné), l'environnement `staging` builde depuis elle, et une pull request de promotion **sans squash** avance `main` en fast-forward jusqu'à `staging` quand le lot est validé.

La promotion n'est délibérément pas un squash : `main` doit recevoir le commit soigné de chaque fonctionnalité individuellement, sinon release-please perd la granularité par changement dont il a besoin pour le changelog et le calcul de version. Comme `staging` n'est jamais qu'en avance sur `main`, la promotion est toujours un fast-forward — `main` reste exactement aussi linéaire que dans le modèle mono-tronc, juste un cran derrière `staging`.
...
### Pourquoi le gitmoji se place après les deux-points

Chaque sujet commence par un emoji, fixé par type (`feat(api): ✨ …`). C'est une aide à la lecture dans `git log` et dans le changelog, rien de plus. Comme la table associe exactement un emoji à chaque type, il n'y a rien à choisir ni à mémoriser — commitlint nomme le bon quand il manque.

Il doit se placer **après** les deux-points. Conventional Commits impose le type en premier, et le parseur de release-please est strict là-dessus : un en-tête qui commence par un emoji, ou qui remplace le type par un emoji, ne s'analyse pas du tout. Le commit disparaîtrait alors du changelog et du calcul de version en silence — aucune erreur, juste une release qui oublie une fonctionnalité. Après les deux-points, l'emoji fait partie du sujet, qu'aucun parseur n'inspecte.
...
Ce qui *peut* être automatisé, c'est tout ce qui suit ce clic : avec [`PROMOTE_ON_RELEASE`](/grocery/fr/references/1_release_and_versionning#promouvoir-automatiquement-à-la-release), merger la Release PR enchaîne directement sur le déploiement — avec, en option, une pause sur un bouton GitHub « Approve and deploy » quand l'environnement a des required reviewers. La porte se déplace au bon endroit ; elle ne disparaît pas.

### Pourquoi seule `staging` possède une branche, et pas `production`

L'erreur classique de git-flow, c'est une branche par environnement : merger dans `staging` déploie staging, merger dans `main` déploie la production, et les deux branches deviennent lentement des espaces de travail — un hotfix atterrit sur l'une et pas l'autre, et l'historique cesse de dire la vérité.

Ce projet garde exactement une branche d'intégration, `staging`, et la traite comme un espace de travail *volontairement* — c'est tout son rôle : un endroit pour assembler et valider un lot avant qu'il ne devienne publiable. Ce qu'il ne fait **pas**, c'est laisser `staging` diverger de `main` : la promotion est toujours un fast-forward, donc les deux branches ne portent jamais des versions différentes du même changement. `production` ne possède aucune branche — elle tourne un tag de version épinglé, promu explicitement. `main` produit des artefacts (builds, images, tags) ; la branche `staging` produit le build staging ; les tags de release alimentent la production. Si un autre hébergeur exige vraiment une branche à surveiller pour la production, une branche `deploy/production` peut exister comme **pointeur avancé en fast-forward** vers un tag — jamais un espace de travail, jamais de commit direct.
...
flowchart TD
    subgraph LOCAL["1 · Committer (local)"]
        A["Branche tirée de staging · feat/…<br/>Commits WIP conventionnels<br/>(lefthook + commitlint)"]
    end

    subgraph PR["2 · La Pull Request (base : staging)"]
    ...
    subgraph MERGE["3 · Le merge (squash → staging)"]
        C["N'importe quel chemin (UI / gh) :<br/>GitHub matérialise titre + description<br/>dans le commit de squash (#123)"]
    end

    subgraph STG["staging"]
        S["Branche d'intégration · l'environnement staging builde depuis elle<br/>Un commit soigné par PR"]
    end

    subgraph PROMO["4 · PR de promotion (staging → main, sans squash)"]
        P["Fast-forward de main jusqu'à staging<br/>chaque commit de fonctionnalité atterrit individuellement"]
    end

    subgraph MAIN["main"]
        D["Tronc de release · historique linéaire<br/>release-please surveille ici"]
    end

    subgraph RELPR["5 · La Release PR (release-please)"]
    ...
    subgraph REL["6 · La release (tag vX.Y.Z)"]
...
2. **Un canon par étape, jamais recopié** : `CONTRIBUTING.md` pour les commits et les PR, le runbook pour les releases du boilerplate, la page [Release et versionnement](/grocery/fr/references/1_release_and_versionning) pour le versionnement et les déploiements. Les skills et les règles pointent vers ces documents ; les dupliquer les ferait dériver.

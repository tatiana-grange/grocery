...
### Why the gitmoji sits after the colon

Every subject opens with one emoji, fixed per type (`feat(api): ✨ …`). It is a scanning aid in `git log` and in the changelog, nothing more. Because the table maps exactly one emoji to each type, there is nothing to pick and nothing to memorize — commitlint names the right one when it is missing.

It has to sit **after** the colon. Conventional Commits puts the type first and release-please's parser is strict about it: a header that opens with an emoji, or that replaces the type with one, does not parse at all. The commit would then drop out of the changelog and out of the version math in silence — no error, just a release that forgets a feature. After the colon, the emoji is part of the subject, which no parser inspects.

...
In this system, no environment owns a branch. `main` produces artifacts (builds, images, tags) and each environment points at an artifact stream — which stream is a per-environment choice, described in [Release and versioning](/lonestone-boilerplate/references/1_release_and_versionning). If a host really requires a branch to watch, a `deploy/<env>` branch can exist as a **fast-forwarded pointer** to a tag — never a workspace, never committed to directly. That is an escape hatch, not the default.
...
2. **One canon per stage, never restated**: `CONTRIBUTING.md` for commits and PRs, the runbook for boilerplate releases, the [Release and versioning](/lonestone-boilerplate/references/1_release_and_versionning) page for versioning and deploys. Skills and rules point to these documents; duplicating them would drift.

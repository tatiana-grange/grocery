...
Do not assign a whole DTO onto an entity whose fields live elsewhere. If, say, some fields live on a child version entity and others arrive as names that must be resolved to references, `wrap(entity).assign(updateDto)` would silently skip them. Assign only what belongs on that entity; keep versioning and collection rebuilds as explicit steps.

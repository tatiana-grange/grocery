...
1. **Onboard once** — if the project isn't tracked yet:
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/lonestone/grocery/main/install.sh \
     | sh -s -- onboard
   ```
...
3. **Apply intentions** — one at a time, with a human confirming apply/skip. Follow the [upgrade runbook](https://github.com/tatiana-grange/grocery/blob/main/.boilerstone/docs/upgrade-runbook.md). The Claude Code / Cursor `boilerstone-upgrade` skills follow that same runbook.
...
For the why behind the system and what each command does, read [how it works](https://github.com/tatiana-grange/grocery/blob/main/.boilerstone/docs/how-it-works.md).

:::note[Working on the boilerplate itself?]
Read the [`.boilerstone/` overview](https://github.com/tatiana-grange/grocery/blob/main/.boilerstone/README.md). Publishing a release is a different procedure: the [release maintainer runbook](https://github.com/tatiana-grange/grocery/blob/main/.boilerstone/docs/release-maintainer-runbook.md).
:::

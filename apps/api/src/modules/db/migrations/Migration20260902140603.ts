...
export class Migration20260902140603 extends Migration {
  ...
  override down(): void | Promise<void> {
    this.addSql(`create table "comment" ("authorName" varchar(255) null, "content" varchar(255) not null, "createdAt" timestamptz(6) not null, "id" uuid not null default gen_random_uuid(), "parentId" uuid null, "postId" uuid not null, "userId" uuid null, primary key ("id"));`);
    ...
    this.addSql(`create table "post" ("coverImage" varchar(255) null, "createdAt" timestamptz(6) not null, "id" uuid not null default gen_random_uuid(), "likesCount" int not null default 0, "publishedAt" timestamptz(6) null, "slug" varchar(255) null, "updatedAt" timestamptz(6) not null, "userId" uuid not null, primary key ("id"));`);
    ...
    this.addSql(`create table "postVersion" ("content" jsonb null, "createdAt" timestamptz(6) not null, "id" uuid not null default gen_random_uuid(), "postId" uuid not null, "title" varchar(255) not null, primary key ("id"));`);
    this.addSql(`create index "postVersion_title_index" on "postVersion" ("title");`);

    this.addSql(`create table "post_tag" ("post" uuid not null, "tag" uuid not null, primary key ("post", "tag"));`);

    this.addSql(`create table "tag" ("id" uuid not null default gen_random_uuid(), "name" varchar(255) not null, "slug" varchar(255) not null, primary key ("id"));`);
    ...
    this.addSql(`alter table "post_tag" add constraint "post_tag_post_foreign" foreign key ("post") references "post" ("id") on update cascade on delete cascade;`);
    this.addSql(`alter table "post_tag" add constraint "post_tag_tag_foreign" foreign key ("tag") references "tag" ("id") on update cascade on delete cascade;`);

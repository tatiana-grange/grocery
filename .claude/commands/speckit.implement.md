...
9. **🚨 MANDATORY: Commit After Each Phase** - This step is NON-NEGOTIABLE:
   ...
   **You MUST commit after completing each phase.** Do not proceed to the next phase without committing.
   
   **a) Update CHANGELOG.md FIRST** - Append an entry to `FEATURE_DIR/CHANGELOG.md`:
   ```markdown
   ## [YYYY-MM-DD HH:MM] - /speckit.implement
   ### Changed
   - Completed Phase [N]: [Phase name]
   - Tasks completed: [list task IDs, e.g., T001, T002, T003]
   - **Author**: AI (Claude)
   ...
   **⛔ STOP: Do NOT start the next phase until the commit is created.**
   ...
   This ensures:
   - Each phase is atomic and traceable in git history
   - Work can be reviewed incrementally
   - Rollback is possible at phase boundaries
